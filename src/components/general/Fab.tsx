...
const Fab = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ icon, iconPosition = 'start', className, children, ...props }, ref) => (
    <button
      ...
      ref={ref}
      className={cn(
        `fixed bottom-4 end-4 z-10 flex h-12 cursor-default select-none
items-center justify-center gap-2 self-center overflow-hidden rounded-lg
...
dark:focus-visible:bg-slate-800 dark:active:bg-slate-800 [&>.icon]:text-xl`,
        isValidElement(icon) && !children ? 'w-12' : 'px-4',
        className
      )}
    >
